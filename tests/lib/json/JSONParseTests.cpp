...
TEST(JSONParse, EmptyString) {
  ...
  EXPECT_EQ(json->getType(), JSONType::LITERAL);
...
TEST(JSONParse, Number) {
  ...
  EXPECT_EQ(json->getType(), JSONType::NUMBER);
...
TEST(JSONParse, Boolean) {
  ...
  EXPECT_EQ(json->getType(), JSONType::BOOLEAN);
...
TEST(JSONParse, SimpleArray) {
  ...
  EXPECT_EQ(json->as<JSONArray>()->at(0)->getType(), JSONType::LITERAL);
...
TEST(JSONParse, ArrayNestedArrays) {
  ...
  EXPECT_EQ(parsed->getType(), JSONType::ARRAY);
  EXPECT_EQ(parsed->as<JSONArray>()->size(), 3);
}

TEST(JSONParse, EmptyObject) {
  std::shared_ptr<JSONEntity> json = JSONEntity::parse("{}");
  EXPECT_EQ(json->getType(), JSONType::OBJECT);
...
TEST(JSONParse, ArrayNestedObject) {
  ...
  EXPECT_EQ(json->getType(), JSONType::ARRAY);
  EXPECT_EQ(json->as<JSONArray>()->size(), 1);
  EXPECT_EQ(json->as<JSONArray>()->at(0)->getType(), JSONType::OBJECT);
...
TEST(JSONParse, ArrayMultipleNestedObjects) {
  ...
  EXPECT_EQ(json->as<JSONArray>()->at(0)->as<JSONObject>()->at("key")->as<
      JSONLiteral>()->value(),
            "value");
  EXPECT_EQ(json->as<JSONArray>()->at(1)->getType(), JSONType::OBJECT);
