...
void Router::get(std::string path, std::function<void()> callback) {
  this->addRoute(std::move(path), HTTPMethod::GET, std::move(callback));
}

void Router::post(std::string path, std::function<void()> callback) {
  this->addRoute(std::move(path), HTTPMethod::POST, std::move(callback));
}

void Router::put(std::string path, std::function<void()> callback) {
  this->addRoute(std::move(path), HTTPMethod::PUT, std::move(callback));
}

void Router::del(std::string path, std::function<void()> callback) {
  this->addRoute(std::move(path), HTTPMethod::DELETE, std::move(callback));
}

void Router::patch(std::string path, std::function<void()> callback) {
  this->addRoute(std::move(path), HTTPMethod::PATCH, std::move(callback));
}

void Router::options(std::string path, std::function<void()> callback) {
  this->addRoute(std::move(path), HTTPMethod::OPTIONS, std::move(callback));
}

void Router::head(std::string path, std::function<void()> callback) {
  this->addRoute(std::move(path), HTTPMethod::HEAD, std::move(callback));
}

void Router::connect(std::string path, std::function<void()> callback) {
  this->addRoute(std::move(path), HTTPMethod::CONNECT, std::move(callback));
}

void Router::trace(std::string path, std::function<void()> callback) {
